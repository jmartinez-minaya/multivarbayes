...
```{r}
## Step 6: Posterior Predictive Distribution Comparison for New Data Points
# 1. Generate new data points for prediction
# We will use the first 3 rows of the original data as new data points for prediction
newdata <- data[1:3, c("X1", "X2")]

# Generate true values for these new observations
newdata_real <- data[1:3, c("Y1", "Y2", "Y3")]

# 2. Generate predictions using `multivarbayes`
mvbayes_predictions <- predict(fit_mvbayes, newdata = newdata)

# 3. Generate predictions using `brms`
# `posterior_predict` returns a matrix of size (n_samples, n_responses * n_observations)
brms_predictions <- posterior_predict(brms_fit, newdata = newdata)

# 4. Convert predictions to data frames for comparison

# `multivarbayes` simulated predictive samples (stored in mvbayes_predictions$marginals)
mvbayes_predicted_samples <- mvbayes_predictions$simulated$predictive_samples
mvbayes_predicted_df <- data.frame()

for (i in 1:dim(mvbayes_predicted_samples)[1]) {
  for (j in 1:dim(mvbayes_predicted_samples)[2]) {
    current_sample <- mvbayes_predicted_samples[i, j, ]  # Samples for the i-th observation and j-th response
...
# Convert `brms` predictions to long format for comparison
# `brms_predictions` is a matrix with rows as samples and columns as each response variable for each observation
brms_predicted_df <- data.frame()
for (obs in 1:nrow(newdata)) {
  for (resp in 1:ncol(newdata_real)) {
    # Each response variable for each observation is stored in a separate column in `brms_predictions`
    # Calculate the correct column index in `brms_predictions`
    col_index <- (obs - 1) * ncol(newdata_real) + resp
    
    # Extract samples for the current observation and response
    current_sample <- brms_predictions[, col_index]
...
# Combine both data frames into one for comparison
combined_predictions_df <- bind_rows(mvbayes_predicted_df, brms_predicted_df)

# 5. Create a data frame for the real observed values
...
# 6. Plot predictive distributions
